...
fun <T> RuleNode<T, RuleResult>.yes(status: RuleStatus, ruleOutcome: RuleOutcome) = yes(RuleResult(status, ruleOutcome))

fun <T> RuleNode<T, RuleResult>.no(status: RuleStatus) = no(RuleResult(status, null))
fun <T> RuleNode<T, RuleResult>.no(status: RuleStatus, ruleOutcome: RuleOutcome) = no(RuleResult(status, ruleOutcome))
